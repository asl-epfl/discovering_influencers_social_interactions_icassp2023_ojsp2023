...
def get_parser():
    ...
    parser.add_argument('--perturbe', default=-1, type=int,
                        help='each perturbe time every adjacency matrix edge changes 0/1 with p=--perturbe_prob')
    parser.add_argument('--perturbe_prob', default=0.01, type=float,
                        help='perturbation probability for --perturbe')
    parser.add_argument('--num_inf', default=3, type=int,
                        help='number of influential nodes')
    parser.add_argument('--sigma_inf', default=0.5, type=float,
                        help='std for influential nodes (when binomial)')
    ...
    parser.add_argument('--window', default=50, type=int, help='loglikelihood window')
    parser.add_argument('--likelihood_perturbe_time', default=-1, type=int,
                        help='at likelihood_perturbe_time the likelohoods changes completely')
